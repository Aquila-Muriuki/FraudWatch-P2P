...
// .wrangler/tmp/bundle-FEgM7V/middleware-loader.entry.ts
init_modules_watch_stub();
init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_process();
init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_console();
init_performance2();

// .wrangler/tmp/bundle-FEgM7V/middleware-insertion-facade.js
init_modules_watch_stub();
init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_process();
init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_console();
init_performance2();
...
function evaluateRules(tx, rules) {
  ...
  for (const r of rules) {
    ...
    const pushFlag = /* @__PURE__ */ __name((f2) => {
      if (!flags.includes(f2)) flags.push(f2);
    }, "pushFlag");
    if (r.ruleType === "threshold" && payload.field === "amount") {
      const limits = payload.limitByDept || {};
      const deptLimit = tx.department && limits[tx.department] ? Number(limits[tx.department]) : void 0;
    ...
    if (r.ruleType === "regex" && payload.field && payload.pattern) {
      ...
      for (const p2 of patterns) {
        ...
        if (patt.test(value)) {
          triggered.push({
            ruleCode: r.code,
            reason: `Field "${payload.field}" matched pattern "${p2}"`,
            legalRef: payload.legalRef,
            points: severity,
            category,
    ...
    if (r.ruleType === "blacklist" && payload.list && Array.isArray(payload.list)) {
      const list = payload.list.map((x2) => x2.toLowerCase());
...
// ../../core/scoring/riskEngine.ts
async function runRiskEngine(tx, prisma, activeRules = [], vendorIn) {
  if (!tx) throw new Error("runRiskEngine: tx is required");
  const { triggered = [], deterministicScore: rulesScore = 0, flags: rulesFlags = [] } = evaluateRules(tx, activeRules || []);
  ...
  try {
    ...
    if (tx.department === "Procurement" && Number(tx.amount) > 1e5) {
  ...
  deterministicScore = Math.min(100, deterministicScore, 70);
  let aiRes = { aiScore: 0, aiFlags: [], aiDetails: {} };
  try {
    aiRes = await runAIChecks(tx, prisma) || aiRes;
  } catch (err) {
    console.error("AI checks error:", err);
  }
  const aiScore = Math.min(40, Number(aiRes.aiScore ?? 0));
  ...
  const weighted = deterministicScore * WEIGHTS.deterministic + aiScore * WEIGHTS.ai + vendorScore * WEIGHTS.vendor;
  const combined = Math.round(Math.min(100, weighted));
  const riskLevel = combined >= 75 ? "HIGH" : combined >= 45 ? "WARNING" : "LOW";
  const status = combined >= 75 ? "BLOCK" : combined >= 45 ? "REVIEW" : "APPROVE";
  const flags = [
    ...
    ...Array.isArray(rulesFlags) ? rulesFlags.map((f2) => ({ type: "RULE_FLAG", flag: f2 })) : [],
    // AI flags
    ...(aiRes.aiFlags ?? []).map((f2) => ({
      type: "AI",
      rule: f2.name,
      reason: f2.message,
  ...
  if (vendor) {
    try {
      await prisma.vendor.update({
        where: { id: vendor.id },
        data: {
          riskScore: Math.min(100, Math.max(0, result.combined))
          // clamp to 0–100
        }
      });
    } catch (e) {
